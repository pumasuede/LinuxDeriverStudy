...
#define TINY_DATA_CHARACTER	"Yu Xiao\n"
		      ...
		      const unsigned char *buffer, int count)
{
	 ...
	 */
	pr_info("%s - %d bytes ", __func__, count);
	for (i = 0; i < count; ++i)
...
static void tiny_set_termios(struct tty_struct *tty, struct ktermios *old_termios)
{
    ...
    pr_info(" tiny_set_termios c_cflag:%d\n", cflag);
...
static int __init tiny_init(void)
{
	...
	pr_info(DRIVER_DESC " " DRIVER_VERSION " is initialized");
	return retval;
}
